...
typedef struct {
    ...
    XImge_processor_uio_map maps[ MAX_UIO_MAPS ];
} XImge_processor_uio_info;

/***************** Variable Definitions **************************************/
static XImge_processor_uio_info uio_info;
...
static int uio_info_read_name(XImge_processor_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XImge_processor_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XImge_processor_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XImge_processor_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XImge_processor_Initialize(XImge_processor *InstancePtr, const char* InstanceName) {
	XImge_processor_uio_info *InfoPtr = &uio_info;
